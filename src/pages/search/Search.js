...
const Search = () => {
	...
	useEffect(() => {
		findArticleByTitle(q).then((res) => {
			if (minimumValidity) {
				res = res.filter(
					(x) => x.validityScore && x.validityScore >= minimumValidity
				);
			}
			if (!!startDate) {
				res = res.filter(
					(x) => x.date && new Date(x.date) >= new Date(startDate)
				);
			...
			if (endDate) {
				...
				res = res.filter(
					(x) => x.date && new Date(x.date) >= new Date(endDate)
				);
				console.log(endDate);
				console.log(res);
			}
